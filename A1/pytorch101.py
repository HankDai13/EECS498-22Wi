...
def multiples_of_ten(start: int, stop: int) -> Tensor:
    ...
    if first_multiple > stop:
        # 如果第一个10的倍数超出了范围，返回空张量
        x = torch.tensor([], dtype=torch.float64)
    else:
        # 计算范围内最后一个10的倍数
        last_multiple = stop // 10 * 10
        # 创建包含所有10的倍数的张量
        x = torch.arange(first_multiple, last_multiple + 10, 10, dtype=torch.float64)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return x
...
def zero_row_min(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    col_indices = torch.argmin(x, dim=1)
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    sigma = torch.sqrt(((x - mu)**2).sum(dim=0, keepdim=True) / (x.shape[0] - 1)) 
    # 标准化
    return (x - mu) / sigma
    ##########################################################################
    #                            END OF YOUR CODE                            #
