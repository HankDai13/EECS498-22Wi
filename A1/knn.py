...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
        train_flattened = torch.flatten(x_train[i])
        for j in range(num_test):
            test_flattened = torch.flatten(x_test[j])
            # 用向量操作替代第三层循环
            dist = ((train_flattened - test_flattened) ** 2).sum()
            dists[i, j] = dist
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
        ...
        # 广播减法，得到(num_test, )的距离
        diff = x_test_flat - train_flat
        dists[i] = (diff ** 2).sum(dim=1)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    test_square = (x_test_flat **2).sum(dim=1).unsqueeze(0)
    # 使用矩阵乘法计算距离
    mul = x_train_flat @ x_test_flat.t()
    dists = train_square + test_square - 2 * mul
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def knn_cross_validate(
    ...
    for k in k_choices:
        ...
        for i in range(num_folds):
            ...
            x_train_folds_ = x_train_folds[:i] + x_train_folds[i + 1:]
            y_train_folds_ = y_train_folds[:i] + y_train_folds[i + 1:]
            x_train_data = torch.cat(x_train_folds_, dim=0)
            y_train_data = torch.cat(y_train_folds_, dim=0)
